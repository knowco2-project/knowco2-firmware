...
def _execute_boot(maintenance):
    ...
    storage.getmount = lambda path: types.SimpleNamespace(label="CIRCUITPY")
    ...
    usb_cdc.enable = lambda console, data: calls.append(
        ("enable_cdc", console, data)
    )
    usb_cdc.disable = lambda: calls.append(("disable", "usb_cdc"))
...
class BootSecurityTests(unittest.TestCase):
    ...
    def test_retail_boot_exposes_data_without_console_and_hides_storage(self):
        calls = _execute_boot(maintenance=False)
        self.assertIn(("enable_cdc", False, True), calls)
        self.assertNotIn(("enable_cdc", True, True), calls)
    ...
    def test_physical_maintenance_boot_keeps_console_and_storage(self):
        ...
        self.assertIn(("enable_cdc", True, False), calls)
