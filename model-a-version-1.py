...
def apply_settings():
    ...
    global cloud_enabled, cloud_api_url, cloud_device_secret, cloud_interval_sec
    ...
    cloud_device_secret = (settings.get("cloud_device_secret", "") or "").strip()
    try:
...
def update_settings_from_params(params):
    ...
    if "cloud_device_secret" in params and params["cloud_device_secret"]:
        settings["cloud_device_secret"] = params["cloud_device_secret"]
  ...
  <div class="wrap">
    ...
    <form method="GET" action="/">
      ...
      <fieldset>
        ...
        <label>Device secret (secret)
          <input type="password" name="cloud_device_secret" maxlength="128" value="">
        </label>
...
def hmac_sha256_hex(key_str, msg_bytes):
    if _HAS_HMAC:
        return hmac.new(key_str.encode("utf-8"), msg_bytes, hashlib.sha256).hexdigest()
    ...
    return "".join("%02x" % b for b in outer)


def hmac_sha256_b64(key_b64_str, msg_bytes):
    """Return base64-encoded HMAC-SHA256 over msg_bytes.
    key_b64_str is base64 string (standard, no newlines)."""
    if not key_b64_str:
        return None
    try:
        # CircuitPython: binascii supports base64 helpers
        key = binascii.a2b_base64(key_b64_str)
    except Exception:
        try:
            import base64 as _b64
            key = _b64.b64decode(key_b64_str)
        except Exception:
            return None

    if _HAS_HMAC:
        mac = hmac.new(key, msg_bytes, hashlib.sha256).digest()
    else:
        if hashlib is None:
            return None
        block = 64
        ...
        o_key_pad = bytes((b ^ 0x5C) for b in k)
        i_key_pad = bytes((b ^ 0x36) for b in k)
        inner = hashlib.sha256(i_key_pad + msg_bytes).digest()
        mac = hashlib.sha256(o_key_pad + inner).digest()

    # binascii.b2a_base64 adds newline; strip it
    try:
        return binascii.b2a_base64(mac).strip().decode("utf-8")
    except Exception:
        try:
            import base64 as _b64
            return _b64.b64encode(mac).decode("utf-8")
        except Exception:
            return None
...
def cloud_send(payload_dict):
    ...
    if not cloud_api_url or not cloud_device_secret:
        return False
    ...
    # Require a valid wall-clock for auth timestamps
    try:
        if not ntp_synced:
            show_status("Time sync needed")
            return False
    except Exception:
        pass

    device_id = (settings.get("device_id") or "co2-node-1").strip()

    ts = int(time.time())
    body = json.dumps(payload_dict)
    msg = (str(ts) + "." + body).encode("utf-8")
    sig = hmac_sha256_b64(cloud_device_secret, msg)
    if not sig:
    ...
    try:
        ...
        log("cloud", "cloud_send error:", e, min_interval=10.0)
